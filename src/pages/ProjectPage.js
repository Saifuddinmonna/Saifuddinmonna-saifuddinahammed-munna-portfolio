...
const ProjectPage = () => {
    ...
    if (!project) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 p-6 text-center">
                ...
                <h1 className="text-4xl sm:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-pink-500 mb-6">Project Not Found</h1>
                 <p className="text-lg text-gray-600 mb-10 max-w-md">Could not find project: "{projectName}". Please check the name or go back.</p>
                <Link
                    to="/"
    ...
    const technologies = project.technology[0] ? project.technology[0].split(',').map(tech => tech.trim()) : [];
        ...
        <div className="bg-gray-100 min-h-screen py-12 md:py-16 lg:py-20 px-4 sm:px-6 lg:px-8">
            <div className="max-w-5xl mx-auto bg-white shadow-2xl rounded-xl overflow-hidden">
                ...
                <div className="p-6 sm:p-8 md:p-10 lg:p-12">
                    <h1 className="text-4xl sm:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-cyan-500 mb-2">{project.name}</h1>
                    <p className="inline-block bg-cyan-100 text-cyan-700 px-3 py-1 rounded-full text-sm font-semibold mb-8 tracking-wide">{project.category}</p>
                        ...
                        <section className="mb-10 md:mb-12">
                            ...
                            <div className="flex flex-wrap gap-3">
                                {technologies.map((tech, index) => (
                                    <span key={index} className="bg-indigo-100 text-indigo-700 px-4 py-1.5 rounded-md text-sm font-medium shadow-sm hover:bg-indigo-200 transition-colors">
                                        {tech}
                                    </span>
                                ))}
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                {project.image.map((img, index) => (
                                   (() => {
                                        ...
                                        console.log(`[ProjectPage] Attempting to load Screenshot ${index + 1} from:`, screenshotPath);
                                        return (
                                            <img
                                                key={index}
                                                src={screenshotPath}
                    ...
                    <section className="mb-10 md:mb-12">
                        ...
                        <div className="space-y-4">
                            {project.liveWebsite && <ProjectLink href={project.liveWebsite} icon={<FaExternalLinkAlt />} text="Live Website" />}
                            {project.liveWebsiteRepo && <ProjectLink href={project.liveWebsiteRepo} icon={<FaGithub />} text="Client-Side Repository" />}
                            {project.liveServersite && <ProjectLink href={project.liveServersite} icon={<FaExternalLinkAlt />} text="Server Site (Live)" />}
                            {project.liveServersiteRepo && <ProjectLink href={project.liveServersiteRepo} icon={<FaGithub />} text="Server-Side Repository" />}
                        </div>
                    </section>
...
const ProjectLink = ({ href, icon, text }) => (
    ...
    >
        {React.cloneElement(icon, { className: "mr-3 w-5 h-5 text-blue-500 group-hover:text-blue-600 transition-colors" })}
        {text}
    </a>
);
