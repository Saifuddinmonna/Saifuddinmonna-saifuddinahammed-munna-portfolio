...
const NavbarPage2 = () => {
  ...
  return (
    ...
    <div className="relative ">
      <nav
        className={`fixed w-full absolute z-50 transition-all duration-300 ${
          scrolled
            ? "bg-white/95 dark:!bg-gray-900/95 backdrop-blur-sm shadow-[0_4px_20px_-4px_rgba(0,0,0,0.1)] dark:!shadow-[0_4px_20px_-4px_rgba(0,0,0,0.3)] hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.15)] dark:!hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.4)]"
            : "bg-white dark:!bg-gray-900 shadow-[0_2px_10px_-2px_rgba(0,0,0,0.05)] dark:!shadow-[0_2px_10px_-2px_rgba(0,0,0,0.2)]"
        }`}
      >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 overflow-x-hidden">
          ...
          <div className="flex justify-between items-center h-14">
            ...
            {/* Desktop Menu */}
            <div className="hidden  bg-white dark:bg-gray-800  text-gray-800 dark:text-gray-50 md:flex items-center space-x-1">
              {navItems.map(item => (
                <Link
                  key={item.label}
                  to={item.path}
                  className={` bg-white dark:bg-gray-800  text-gray-800 dark:text-gray-50 relative px-3 py-2 no-underline text-lg font-bold dark:text-gray-200 hover:text-blue-600 dark:hover:text-blue-400 transition-all duration-300 group ${
                    isActive(item.path) ? "text-blue-600 dark:text-blue-400" : ""
                  }`}
                >
                  {item.label}
                  <span
                    className={` absolute bottom-0 left-1/2 transform -translate-x-1/2 w-[calc(100%-1.5rem)] h-0.5 bg-gradient-to-r from-blue-600 to-cyan-500 dark:!from-blue-400 dark:!to-cyan-300 transition-all duration-300 ${
