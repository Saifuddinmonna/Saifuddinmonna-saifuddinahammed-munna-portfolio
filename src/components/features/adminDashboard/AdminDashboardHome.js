        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
          {dashboardStats.map((stat, index) => (
            <motion.div
              ...
              className={`bg-[var(--background-paper)] rounded-lg p-6 shadow-lg border border-[var(--border-color)] hover:shadow-xl transition-all duration-300 ${
                stat.path ? "cursor-pointer hover:scale-105" : ""
              }`}
              onClick={() => {
                if (stat.path) {
                  window.location.href = stat.path;
                }
              }}
            >
              ...
              <div>
                ...
                <p className="text-xs text-[var(--text-secondary)]">{stat.description}</p>
              </div>
            </motion.div>
          ))}
        </div>
